"""Build the five-partition ORIGINAL-RESTORE-0.2.2 package, PC only.
First --operation restore --build-only; then --operation restore.
Preserves existing userdata; never invokes or embeds a data formatter.
All build/release outputs are new. Original release 0.2.0 stays untouched.
"""
import argparse
import hashlib
import json
...
OUTPUT = HERE/'salida'
GO_SOURCES = ('block_layout.go','block_layout_test.go','block_device_linux.go','go.mod','package.go','package_test.go','main_linux.go','main_windows.go','block_abi.go','block_abi_test.go','backup_verified.go','backup_verified_test.go','transaction_linux.go','env_guard.go','env_guard_test.go','env_read_linux.go')
...
def package_id(operation):
    return 'TVBASE-P291-A9-0.2.2' if operation=='install' else 'TVBASE-P291-A9-ORIGINAL-RESTORE-0.2.2'
...
def reviewed_images(report, originals):
    ...
    require(review['version']==VERSION and review['package_id']==package_id('install') and review['dt_id']=='gxlx2_p291_1g','Wrong reviewed revision')
...
def build(operation):
    ...
    operation_id='install_reviewed' if operation=='install' else 'restore_original'
...
def layout_review_proof():
    review_path=HERE/'REVISION-LAYOUT-022.json'
    review=json.loads(review_path.read_text(encoding='utf-8'))
    require(review['state']=='passed','Independent layout review did not pass')
    expected={p.name:sha(p) for p in HERE.glob('*.go')}
    expected['go.mod']=sha(HERE/'go.mod')
    require(set(expected)==set(GO_SOURCES),'An unlisted Go source would change the compiled installer')
    require(review['source_sha256']==expected,'Layout review does not match all Go sources')
    require(review['builder_sha256']==sha(HERE/'empaquetar_original.py'),'Builder changed after independent review')
    return {'file':review_path.relative_to(ROOT).as_posix(),'sha256':sha(review_path),
            'state':'passed','start_offsets_originally_captured':False,
            'observed_layout_frozen_and_revalidated':True}


def package(operation,report):
    layout_review=layout_review_proof()
    ident=package_id(operation);directory=BUILD_ROOT/ident
    ...
    operation_id='install_reviewed' if operation=='install' else 'restore_original'
    manifest={'format':2,'id':ident,'dt_id':'gxlx2_p291_1g','media_id':MEDIA,'operation':operation_id,
              'data_policy':'fresh_userdata_required' if operation=='install' else 'preserve_existing_userdata','images':[]}
    ...
    result={'layout_review':layout_review,'version':VERSION,'package_id':ident,'operation':operation_id,'file':final.relative_to(ROOT).as_posix(),
            ...
            'data_policy':manifest['data_policy'],'automatic_userdata_wipe':False,
            'physically_installed':False,'oem_recovery_acceptance_confirmed':False,'restore_path_tested':False,
            'pending_before_install':['recovery_entry_and_package_path','separate_explicit_userdata_migration' if operation=='install' else 'explicit_restore_decision'],
            'limits':['Non-A/B; no automatic rollback','Test key and legacy SHA1 are compatibility measures, not production update security',
                      'Restore package restores five OEM partitions including their original software; it does not sanitize userdata']}
...
def main():
    ...
    parser.add_argument('--operation',choices=('restore',),required=True)
