...
func rejectIncompleteMigration(usb string) error {
	...
	for _, entry := range entries {
		if !strings.HasPrefix(entry.Name(), "TVBASE-respaldo-rest021-") && !strings.HasPrefix(entry.Name(), "TVBASE-respaldo-rest022-") {
			continue
		}
...
func backupFive(ts []Target, usb string) (string, []BackupEvidence, error) {
	...
	dir, e := os.MkdirTemp(usb, "TVBASE-respaldo-rest022-")
	if e != nil {
		return "", nil, e
	}
...
func persistBackupSet(dir string, records []BackupEvidence, ts []Target) error {
	...
	return writeDurableJSON(filepath.Join(dir, "00-backup-verified.json"), map[string]any{
		...
		"layouts_observed": observedLayouts, "start_offsets_originally_captured": false, "records": records, "userdata_untouched": true, "created_utc": time.Now().UTC().Format(time.RFC3339),
	})
}
