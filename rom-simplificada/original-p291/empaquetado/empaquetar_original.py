...
First: --operation install --build-only (or restore).
Then: --operation install --revision-report <reviewed JSON>.
Restoration uses only the five verified originals: --operation restore.
...
OUTPUT = HERE/'salida'
GO_SOURCES = ('go.mod','package.go','data_policy.go','package_test.go','main_linux.go','main_windows.go')
...
def package_id(operation):
    return 'TVBASE-P291-A9-0.2.0' if operation=='install' else 'TVBASE-P291-A9-ORIGINAL-RESTORE-0.2.0'
...
def main():
    ...
    parser.add_argument('--operation',choices=('install','restore'),required=True)
