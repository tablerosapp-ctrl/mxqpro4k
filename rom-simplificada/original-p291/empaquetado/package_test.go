...
func reviewedFixtureManifest() Manifest {
	...
	for _, name := range names {
		...
		m.Images = append(m.Images, Image{Name: name, Entry: "tvbase/" + name + ".img", Size: g.Size, MajorMinor: g.MajorMinor, SHA256: hex.EncodeToString(make([]byte, 32)), OriginalSHA256: hex.EncodeToString(make([]byte, 32))})
	}
	return m
}
