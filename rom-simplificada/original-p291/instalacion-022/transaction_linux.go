...
func rejectIncompleteMigration(usb string) error {
	...
	for _, entry := range entries {
		if !strings.HasPrefix(entry.Name(), "TVBASE-respaldo-021-") && !strings.HasPrefix(entry.Name(), "TVBASE-respaldo-022-") {
			continue
		}
		...
		_, started := os.Lstat(filepath.Join(dir, "10-format-started.json"))
		_, finished := os.Lstat(filepath.Join(dir, "90-installed-verified.json"))
...
func targetSetGuard(ts []Target, usb string) error {
	...
	if e = revalidateBlockLayouts("guarda de respaldo/formato/flash"); e != nil {
		return e
	}
	for _, t := range ts {
		layout, e := inspectBlock(t.Path, t.Image.Name)
		if e != nil {
			return fmt.Errorf("guarda de destino %s: %w", t.Image.Name, e)
		}
		if layout != t.Layout {
			return fmt.Errorf("cambio el layout del destino %s", t.Image.Name)
		}
	}
	return nil
}

func backupSix(ts []Target, usb string) (string, []BackupEvidence, error) {
	var total uint64
	for _, t := range ts {
	...
	if len(ts) != 6 || total != 6067060736 {
		return "", nil, fmt.Errorf("conjunto de seis respaldos inesperado")
	}
	...
	if uint64(fs.Bavail)*uint64(fs.Bsize) < total+backupSpaceMargin {
		return "", nil, fmt.Errorf("se necesitan al menos 6603931648 bytes libres para el respaldo")
	}
	dir, e := os.MkdirTemp(usb, "TVBASE-respaldo-022-")
	if e != nil {
		return "", nil, e
	}
	...
	for _, t := range ts {
		...
		result, e := backupVerifiedFile(t.Image.Name, t.Path, filepath.Join(dir, t.Image.Name+".img"), t.Size, t.Image.OriginalSHA256, hooks)
		if e != nil {
...
func persistBackupSet(dir string, records []BackupEvidence, ts []Target) error {
	...
	return writeDurableJSON(filepath.Join(dir, "00-backup-verified.json"), map[string]any{
		...
		"layouts_observed": observedLayouts, "start_offsets_originally_captured": false, "records": records, "data_bytes": userdataBytes, "data_major_minor": "179:20", "created_utc": time.Now().UTC().Format(time.RFC3339),
	})
}
...
func backupSetStillValid(dir string, records []BackupEvidence, ts []Target, usb string) error {
	...
	for _, t := range ts {
		if t.Image.Name == "data" {
			if e := targetSetGuard(ts, usb); e != nil {
				return e
			}
			say("Verificando que los datos originales no cambiaron antes del formato.")
			sum, e := hashWithRecoveryProgress(t.Path, t.Size, "Comprobando userdata original")
			if e != nil {
				return e
			}
			found := false
			for _, r := range records {
				if r.Name == "data" {
					found = true
					if sum != r.SHA256 {
						return fmt.Errorf("userdata cambio despues del respaldo")
					}
				}
			}
			if !found {
				return fmt.Errorf("falta respaldo data")
			}
		}
	}
