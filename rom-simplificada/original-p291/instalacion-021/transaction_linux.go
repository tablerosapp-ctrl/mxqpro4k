...
func rejectIncompleteMigration(usb string) error {
	...
	for _, entry := range entries {
		if !strings.HasPrefix(entry.Name(), "TVBASE-respaldo-021-") {
			continue
		}
...
func backupSix(ts []Target, usb string) (string, []BackupEvidence, error) {
	...
	dir, e := os.MkdirTemp(usb, "TVBASE-respaldo-021-")
	if e != nil {
		return "", nil, e
	}
...
func persistBackupSet(dir string, records []BackupEvidence, ts []Target) error {
	...
	return writeDurableJSON(filepath.Join(dir, "00-backup-verified.json"), map[string]any{
		...
		"records": records, "data_bytes": userdataBytes, "data_major_minor": "179:20", "created_utc": time.Now().UTC().Format(time.RFC3339),
	})
}
