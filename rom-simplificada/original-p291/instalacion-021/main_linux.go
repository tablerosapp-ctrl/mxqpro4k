...
func checkFreshUserdata(disk string) error {
	return verifyFreshDataAndUnmount(disk)
}
...
func run() error {
	...
	say("TVBASE P291: instalador0.2.1, plataforma0.2.0")
	say("Se respaldan los datos y luego se eliminan del TV para instalar Android limpio.")
	...
	if !strings.HasPrefix(packagePath, rootPath+string(os.PathSeparator)) {
		return fmt.Errorf("el ZIP debe abrirse directamente desde el pendrive marcado")
	}
	...
	say("Verificando todas las imagenes antes de respaldar...")
	lastVerification := time.Time{}
	if e = p.verifyWithProgress(func(im Image, n int64) {
		if time.Since(lastVerification) >= 5*time.Second {
			say(fmt.Sprintf("Verificando ZIP %s: %d/%d bytes", im.Name, n, im.Size))
			lastVerification = time.Now()
		}
	}); e != nil {
		return e
	}
	data, e := dataTarget(ts[0].Disk)
	if e != nil {
		return e
	}
	if e = detachDataNormally(data); e != nil {
		return e
	}
	if e = checkOriginalDataReadable(data); e != nil {
		return e
	}
	all := append(append([]Target{}, ts...), data)
	if e = targetsUnmounted(all); e != nil {
		return e
	}
	var dir string
	var records []BackupEvidence
	return executeMigration(MigrationStages{
		Backup:                func() error { var err error; dir, records, err = backupSix(all, usb); return err },
		PersistVerifiedBackup: func() error { return persistBackupSet(dir, records, all) },
		Format: func() error {
			if e := backupSetStillValid(dir, records, all, usb); e != nil {
				return e
			}
			return formatUserdata(data, dir)
		},
		VerifyFreshUnmounted: func() error {
			if e := verifyFreshDataAndUnmount(data.Disk); e != nil {
				return e
			}
			return writeDurableJSON(filepath.Join(dir, "20-userdata-prepared.json"), map[string]any{"state": "userdata_prepared", "filesystem_bytes": userdataFilesystemBytes, "footer_zero": true, "empty_readonly_mount_verified": true, "unmounted_after_check": true})
		},
		Flash: func() error {
			if e := flash(p, ts, dir); e != nil {
				return e
			}
			if e := writeDurableJSON(filepath.Join(dir, "90-installed-verified.json"), map[string]any{"state": "installed_verified", "package_id": allowedPackageID, "platform_version": "0.2.0"}); e != nil {
				return e
			}
			progress(1)
			say("ROM instalada y releida. Respaldo completo: " + filepath.Base(dir))
