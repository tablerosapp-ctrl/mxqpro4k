...
public class EvidenciaHarness06 {
 ...
 static void guards()throws Exception{
  ...
  for(int n:new int[]{0,1,2,4}){Fake f=new Fake();f.fail=n;reject(new Evidencia(),f,"uid=2000(shell)","gxlx2_p291_1g","28");require(f.seen.size()==n+1,"Failure retried/continued");}
  ...
  require(f.seen.size()==5&&f.seen.get(4)==4&&result.contains("guardada y comprobada"),"Final stage missing");
  reject(e,f,"uid=2000(shell)","gxlx2_p291_1g","28");require(f.seen.size()==5,"Same capture repeated");
  ...
  for(int n=0;n<5;n++){String c=EvidenciaScripts.command(n,n==0?"":DIR,TOKEN);require(c.getBytes("UTF-8").length+7<=4096,"Oversized OPEN");}
