"""Build diagnostic-only 0.6 in a fresh output; never sign with a new key implicitly."""
from pathlib import Path
import hashlib,json,subprocess,zipfile,sys
ROOT=Path(__file__).resolve().parents[2]
SRC=ROOT/'rom-simplificada/componentes/acceso-usb-0.6'
OUT=ROOT/'rom-simplificada/compilacion/acceso-usb-0.6'
...
assert b"versionCode='6'" in badging and b"versionName='0.6'" in badging
...
record={'component':'acceso-usb','version':'0.6','path':final.relative_to(ROOT).as_posix(),'bytes':final.stat().st_size,'sha256':hashlib.file_digest(final.open('rb'),'sha256').hexdigest(),'signer_sha256':'d2136c0e519d477d2be34b55590d9e548137682f9c2573f330a0a6e91833b613','purpose':'diagnostico_sin_reinicio','tv_tested':False}
(OUT/'componente.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf8')
host=ROOT/'rom-simplificada/instalador/host-classes-0.6';host.mkdir(exist_ok=True)
run([JAVA,'-jar',ECJ,'-8','-encoding','UTF-8','-d',host,*[p for p in sources if p.name!='Acceso.java'],ROOT/'rom-simplificada/instalador/EvidenciaHarness06.java'])
