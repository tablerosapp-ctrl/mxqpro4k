...
public class Acceso extends Activity {
  ...
  @Override public void onCreate(Bundle state) {
    ...
    TextView title=new TextView(this);title.setText("Diagnóstico del último intento · TV Base · 0.8");title.setTextSize(25);list.addView(title);
    TextView detail=new TextView(this);detail.setText("Conectá el pendrive TVBASE. Esta versión busca registros del intento anterior y consulta WiFi, Bluetooth, batería, espacio y WebView con límite de tiempo. Guarda los datos y comprueba sus hashes. No cambia WiFi ni Bluetooth. No reinicia, no abre el actualizador y no instala la ROM. No necesita WiFi ni Internet.");detail.setTextSize(18);list.addView(detail);
    collect=button(list,"Guardar diagnóstico del último intento");collect.setEnabled(!attempted);collect.setOnClickListener(new View.OnClickListener(){public void onClick(View v){access(true);}});
    ...
    info=new TextView(this);info.setText(attempted?"Esta captura ya se intentó. Conservá cualquier carpeta TVBASE-postintento del pendrive. No se repetirá automáticamente.":"Android "+android.os.Build.VERSION.RELEASE+" · API "+android.os.Build.VERSION.SDK_INT+"\nPreparado para leer evidencia sin reiniciar. Cada etapa tiene un límite de 90 segundos.");info.setTextSize(16);info.setTextIsSelectable(true);list.addView(info);
  }
