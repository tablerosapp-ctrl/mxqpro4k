"""Two bounded read-only checks, then one explicit OEM menu open. No ZIP selection."""
from pathlib import Path
import json
HERE=Path(__file__).resolve().parent
MARKER='TVBASE-P291-20260906-4dc82786'
ROM='TVBASE-P291-A9-0.1.1-RECOVERY.zip'
ROM_SHA='e7279a7901bc0b513ccc5d3a66a1b5bf483908a4cffd30f34f5d8d463fc95205'
OTA='/product/app/OTAUpgrade/OTAUpgrade.apk'
OTA_SHA='9ffb822fc76974ee9df8c5493f0929638b69140f71506946cb410bab489a1d9c'
COMMON=r'''tvbase_fail() { printf 'ERROR: %s\n' "$1"; exit 9; }
...
'''
DISCOVER=r'''for t_c in /storage/* /mnt/media_rw/*; do
 [ -d "$t_c" ] || continue
...
tvbase_profile
'''
LAUNCH=r'''t_launch=$(/system/bin/am start -W -n com.droidlogic.otaupgrade/.MainActivity 2>&1) || { printf '%s\n' "$t_launch"; tvbase_fail 'apertura rechazada'; }
...
'''
def generate():
 scripts=[DISCOVER,COMMON+VERIFY+LAUNCH]
 out=HERE/'scripts';out.mkdir(exist_ok=True)
 for n,s in enumerate(scripts):(out/f'etapa-{n}.sh').write_text(s,encoding='utf8',newline='\n')
 source='''package local.tvbase.acceso;
import java.io.IOException;
final class EntradaScripts {
 static final String[] BODY={'''+','.join(json.dumps(s,ensure_ascii=True) for s in scripts)+'''};
 static String command(int step,String directory,String token)throws IOException {
  if(!Entrada.validToken(token)||step<0||step>=BODY.length||(step==0&&!"".equals(directory))||(step>0&&!Entrada.validDirectory(directory)))throw new IOException("Etapa o destino inválido");
  String text="tv_t='"+token+"';tv_d='"+directory+"';"+BODY[step];
  if(text.getBytes("UTF-8").length+7>4096)throw new IOException("Comando excede límite ADB");
  return text;
 }
}
'''
 (HERE/'EntradaScripts.java').write_text(source,encoding='utf8')
