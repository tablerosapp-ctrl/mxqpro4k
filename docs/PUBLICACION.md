...
Para el cliente LAN0.9 se clasifican únicamente los tres CIDR RFC1918 completos de su validador y las dos direcciones ficticias exactas en su archivo de pruebas, que usa transporte simulado. La prueba de rechazo de IP pública usa un rango reservado para documentación. La sesión real, nonce, dirección del TV y respuestas se guardan bajo `privado`; no se publican ni se sustituyen por un destino fijo en el cliente.
