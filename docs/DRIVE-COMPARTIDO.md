...
La aplicación sigue en modo Prueba, con la cuenta propietaria registrada. El titular aprobó el [aviso de privacidad vigente](DRIVE-PRIVACIDAD.md), publicado en GitHub y guardado en la configuración de Google junto con la página del repositorio y el dominio de sus enlaces. Se conserva la [propuesta anterior](DRIVE-PRIVACIDAD-PROPUESTA.md).

El botón de producción ya está disponible, pero su diálogo informa que la aplicación podrá ser autorizada por cualquier cuenta Google. La revisión automática rechazó la confirmación porque la aprobación anterior no especificaba esa ampliación de usuarios. **No se cambió a producción ni se eludió el rechazo.** Pendiente de autorización explícita del titular para ese alcance. Esto se refiere a quién puede autorizar la aplicación; no se publicaron archivos de Drive ni se modificaron los permisos de la carpeta.

Mientras siga en Prueba, Google limita el token de renovación a siete días para estos permisos ([fuente oficial](https://developers.google.com/identity/protocols/oauth2#expiration)). No prometer conexión indefinida. Si se autoriza producción y Google aplica requisitos adicionales de verificación, documentarlos antes de afirmar finalización.
