...
func checkedDirectoryFD(path string, dev uint64) (int, error) {
	...
	if e = syscall.Fstat(fd, &st); e != nil || uint64(st.Dev) != dev {
		syscall.Close(fd)
		return -1, fmt.Errorf("directorio no pertenece al USB")
	}
	return fd, nil
}
func newDestination(u usbTarget, parent, name string) (*linuxDestination, error) {
	...
	if e = syscall.Fstat(pfd, &st); e != nil || uint64(st.Dev) != u.Dev {
		return nil, fmt.Errorf("CAPTURAS fuera del USB")
	}
	if e = u.verify(); e != nil {
		return nil, e
	}
	...
	if e = syscall.Fstat(fd, &st); e != nil || uint64(st.Dev) != u.Dev {
		syscall.Close(fd)
		return nil, fmt.Errorf("captura fuera del USB")
	}
...
func (d *linuxDestination) CreateExclusive(name string) (SyncWriteCloser, error) {
	...
	if e != nil || !st.Mode().IsRegular() || uint64(st.Sys().(*syscall.Stat_t).Dev) != d.device {
		f.Close()
		return nil, fmt.Errorf("salida no es archivo ordinario del USB")
	}
	return f, nil
}
...
func (d *linuxDestination) free() (uint64, error) {
	...
	if s.Bsize <= 0 {
		return 0, fmt.Errorf("tamano de bloque USB invalido")
	}
