"""Whole-file OTA RSA/SHA-256 for the supplied CNV8b recovery v3 development key.
...
CERT_SHA256 = 'cf2ab3818595c4eda08cec72bbe39bc377a30b764a3eebde52536eae01808525'
RECOVERY_KEYS_SHA256 = '96a7556cb8f2f377a318def54bfb732a125fead794ad86fbcc9937b8718a36ca'
...
def hash_prefix(path, count, algorithm_name='sha256'):
...
def trust_cnv8b_v3_key(keyfile, cert):
    data = Path(keyfile).read_bytes()
    require(hashlib.sha256(data).hexdigest() == RECOVERY_KEYS_SHA256, 'Supplied CNV8b recovery key file changed')
    require(cert.fingerprint(hashes.SHA256()).hex() == CERT_SHA256, 'Unexpected OTA certificate')
    match = re.fullmatch(rb'v3 \{64,(0x[0-9a-fA-F]+),\{([0-9,]+)\},\{([0-9,]+)\}\}', data.strip())
    require(match is not None, 'Expected exactly one v3 RSA key')
    ...
    require(numbers.e == 3 and modulus.bit_length() == 2048 and modulus == numbers.n, 'RSA key differs from supplied CNV8b recovery')
    require(int(match[1],16) == (-pow(modulus,-1,2**32))%2**32 and rr == pow(2,4096,modulus), 'Invalid recovery RSA parameters')
    return {'key_version': 3, 'rsa_exponent': 3, 'rsa_bits': 2048, 'digest': 'SHA256',
...
def sign_zip(unsigned, final, cert, key):
    ...
    signature = key.sign(hash_prefix(unsigned, size-2), padding.PKCS1v15(), utils.Prehashed(hashes.SHA256()))
    issuer_serial = node(0x30, cert.issuer.public_bytes()+integer(cert.serial_number))
    signer = node(0x30, integer(1)+issuer_serial+algorithm(OID_SHA256)+algorithm(OID_RSA)+node(4,signature))
    signed_data = node(0x30, integer(1)+node(0x31,algorithm(OID_SHA256))+node(0x30,node(6,OID_DATA))+
    ...
    message = b'TVBASE RK1 supplied CNV8b recovery v3; public development key\0'
...
def verify_zip(path, cert):
    ...
    require(len(data) == 5 and data[1][2] == node(0x31,algorithm(OID_SHA256)), 'Digest set must contain only SHA256')
    ...
    require(len(info) == 5 and info[2][2] == algorithm(OID_SHA256) and info[3][2] == algorithm(OID_RSA) and info[4][0] == 4, 'Signer must use direct RSA/SHA256 without attributes')
    ...
    cert.public_key().verify(info[4][1],hash_prefix(path,signed_bytes),padding.PKCS1v15(),utils.Prehashed(hashes.SHA256()))
    return {'signature_algorithm':'RSA-PKCS1v1.5/SHA256','signed_bytes':signed_bytes,
            'signed_region_sha256':hash_prefix(path,signed_bytes,'sha256').hex(),
            'sha256_policy_replay_verified':True,'certificate_sha256':cert.fingerprint(hashes.SHA256()).hex(),
