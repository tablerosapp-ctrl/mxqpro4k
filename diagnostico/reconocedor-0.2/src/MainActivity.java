...
public final class MainActivity extends Activity {
    ...
    private final Runnable refresh=new Runnable(){public void run(){
        ...
        scan.setEnabled(!busy&&!HardwareCollector.hasPendingReads());choose.setEnabled(!busy);save.setEnabled(!busy);label.setEnabled(!busy);
    ...
    public void onCreate(Bundle saved){
        ...
        TextView title=new TextView(this);title.setText("Reconocimiento TV Base · 0.2");title.setTextColor(0xffffffff);title.setTextSize(24);column.addView(title);
        TextView intro=new TextView(this);intro.setText("Primero guarda una ficha inicial en USB; luego el inventario del equipo.\nLa copia profunda se prepara por separado desde recovery. No instala una ROM.");intro.setTextColor(0xffe1e7f0);intro.setTextSize(16);column.addView(intro);
        label=new EditText(this);label.setSingleLine(true);label.setTextColor(0xffffffff);label.setHintTextColor(0xffbfc9dc);label.setHint("Nombre opcional, por ejemplo MX9 5G");label.setText(prefs.getString("display_name",""));column.addView(label);
        choose=button(column,"Elegir pendrive (si no se detecta automáticamente)");scan=button(column,"Capturar y guardar este equipo");save=button(column,"Volver a guardar la última ficha terminada");
    ...
    void selectUsb(){try{Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT_TREE);i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_GRANT_WRITE_URI_PERMISSION|Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION|Intent.FLAG_GRANT_PREFIX_URI_PERMISSION);startActivityForResult(i,7);}catch(Exception e){message("No hay selector de carpetas: "+error(e));}}
    ...
    void startCapture(){
        ...
        worker.execute(new Runnable(){public void run(){
            ...
            try{
                ...
                CaptureSequence.run(new CaptureSequence.Steps(){
                    ...
                    public void saveInventory()throws Exception {
                        message("Ficha inicial guardada. Consultando inventario; conservá el USB conectado…");
            ...
            }catch(Throwable e){message("Guardado detenido: "+error(e)+"\n"+(baselineExported[0]?"La ficha inicial ya está en el pendrive. ":"")+"Podés usar Volver a guardar la última ficha terminada. No se instala ni borra nada.");}
    ...
    JSONObject readBaseline(ReportArchive.Result archive)throws Exception {
        try(java.util.zip.ZipFile zip=new java.util.zip.ZipFile(archive.archive)){
            ...
            if(!"basic_checkpoint".equals(base.getString("capture_state"))||!"0.2".equals(base.getString("recognizer_version"))||!deviceId.equals(base.getString("device_id")))throw new IOException("La ficha no corresponde a esta instalación");
            return base;
        }
    }
    ...
    void exportArchive(ReportArchive.Result a,String name,boolean terminal)throws Exception {
        ...
        else{File folder=UsbStore.automatic();if(folder==null)throw new IOException("Ficha local lista. Elegí el pendrive y pulsá Volver a guardar");result=UsbStore.direct(folder,a,cb);}
