"""Host-only sequence failure barriers and immutable ZIP fixtures; never accesses USB."""
import hashlib
import json
...
def main():
    sources = [HERE / 'src/CaptureSequence.java', HERE / 'src/ReportArchive.java',
               HERE / 'tests/CaptureSequenceHarness.java', Path(__file__).resolve()]
    def hashes():
    ...
    output = Path(tempfile.mkdtemp(prefix='test-sequence-', dir=private))
    classes = output / 'classes'
    classes.mkdir()
    run([JAVA, '-jar', ECJ, '-8', '-encoding', 'UTF-8', '-cp', JSON, '-d', classes, *sources[:3]])
    result = json.loads(run([JAVA, '-cp', str(classes) + ';' + str(JSON),
                             'com.tvbase.reconocimiento.CaptureSequenceHarness', output / 'fixture']))
    if before != hashes():
        raise RuntimeError('Source changed during host tests')
    result['source_sha256'] = before
    result['android_ui_tested'] = False
    result['saf_tested'] = False
    result['usb_tested'] = False
