"""Importador 0.2: verifica capturas y conserva copias privadas nuevas en PC.
...
def reported_capture_state(report):
    """Prefer the canonical capture_state; retain legacy field provenance.

    A valid container can describe a partial, failed, or basic-only capture.
    Unknown textual states are preserved rather than upgraded to completion.
    """
    for key in ('capture_state', 'capture_status', 'status'):
        if key in report:
            value = report[key]
            need(isinstance(value, str) and 0 < len(value) <= 128
                 and not any(unicodedata.category(c) in ('Cc', 'Cs') for c in value),
                 'Estado de captura presente pero inválido.')
    for key in ('capture_state', 'capture_status', 'status'):
        if key in report:
...
def validate_report(report, filename):
    ...
    reported_capture_state(report)
    if 'parent_capture_id' in report:
        parent = uuid_text(report['parent_capture_id'])
        need(parent != capture, 'Una captura no puede ser su propia ficha anterior.')
...
def catalog(items, copies):
    ...
    for item, copied in zip(items, copies):
        ...
        profile = report['suggested_profile']
        capture_state, state_field = reported_capture_state(report)
        ...
        captures.append({'capture_id': report['capture_id'], 'device_id': identity,
                         ...
                         'reported_capture_status': capture_state,
                         'reported_capture_status_source': state_field,
                         'parent_capture_id': report.get('parent_capture_id'),
    ...
    return {'schema': 'tvbase-recognition-import-1', 'importer_version': '0.2',
            'state': 'imported_verified', 'created_at_utc': now(),
