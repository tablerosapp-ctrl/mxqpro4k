...
func run() error {
	...
	say("TV Base - Extraccion de originales 0.3 en SD. No instala ni formatea.")
	sd, e := findSD()
	if e != nil {
		return e
	}
	say("SD verificada. Leyendo mapa real eMMC; no se monta ni modifica la memoria interna.")
	dt := readDT()
	plan, e := loadPlan(sd.Root, dt)
	if e != nil {
		return e
	}
	...
	if e = separateDestination(sd.Proof, facts); e != nil {
		return e
	}
	...
	chosen, selectedSkips, e := chooseRK3229CSources(facts, mounts, dt, ident, rk3229CExpectedCID)
	if e != nil {
		return e
	}
	...
	parent := filepath.Join(sd.Root, "CAPTURAS")
	if _, e = ordinary(parent, true); e != nil {
		return e
	}
	if e = sd.verify(); e != nil {
		return e
	}
	output := filepath.Join(parent, "TVBASE-"+profile+"-"+captureID)
	dest, e := newDestination(sd, parent, filepath.Base(output))
	if e != nil {
		return e
	}
	...
	info := map[string]any{"schema": "tvbase-recovery-inventory-1", "destination": sd.Proof, "capture_id": captureID, "profile_plan": plan, "dt_identity": dt, "emmc_identity_hashes": ident, "blocks": facts, "mounts": mounts, "omitted": skips, "captured_wall_time": time.Now().UTC().Format(time.RFC3339), "wall_clock_trusted": false, "coverage": "Only reported accessible areas; not RPMB, a tested restore, malware attestation, or universal device support."}
	info["source_policy"] = rk3229CPolicyProof()
	...
	revalidate := func() error {
		...
		if e = separateDestination(sd.Proof, now); e != nil {
			return e
		}
		ms, e := currentMounts()
		if e != nil {
		...
		cs, _, e := chooseRK3229CSources(now, ms, baselineDT, nowIdent, rk3229CExpectedCID)
		if e != nil {
			return e
		}
	...
	last := time.Time{}
	report, e := Capture(cp, CaptureHooks{BufferedBackup: true, AvailableMemory: func() (uint64, error) {
		say("Backup al final: comprobando memoria disponible/estimada para dos lecturas antes de escribir SD.")
		data, e := bounded("/proc/meminfo", 65536)
		if e != nil {
			return 0, e
		}
		return parseMemoryAvailable(data)
	}, OpenSource: func(s Source) (io.ReadCloser, error) { return openBlock(s.Device, s.MajorMinor, s.Bytes) }, Revalidate: revalidate, FreeBytes: dest.free, Destination: dest, Progress: func(p Progress) {
	...
	say("Archivos guardados en SD: TVBASE-EXTRACCION/CAPTURAS. No se reinicia automaticamente.")
	return nil
}
func main() {
	if len(os.Args) == 2 && os.Args[1] == "--help" {
		fmt.Println("TV Base extractor 0.3 SD, solo unidad RK3229-C vinculada; solo protocolo update-binary desde recovery. Sin plan APK coincidente guarda inventario; no instala ROM.")
		return
	}
