...
func chooseSources(facts []BlockFact, mounts []Mount) ([]BlockFact, []Skip, error) {
	return chooseSourcesConfigured(facts, mounts, true)
}

// Choosing individual partitions allows a known unstable source to be tried
// last. This does not change its hash requirements or mark a failed copy valid.
func chooseSourcesConfigured(facts []BlockFact, mounts []Mount, allowWhole bool) ([]BlockFact, []Skip, error) {
	...
	for _, name := range disks {
		...
		if whole {
			...
			reason := "area completa omitida: montaje RW o dispositivo en uso; se evalua cada particion"
			if !allowWhole {
				reason = "area completa omitida por politica de orden: se evalua cada particion y backup se intenta al final"
			}
			skips = append(skips, Skip{name, reason})
