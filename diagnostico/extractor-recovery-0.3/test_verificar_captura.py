...
class VerificationTests(unittest.TestCase):
    ...
    def with_c_policy(self):
        self.with_sd_destination()
        self.inventory["dt_identity"] = "rockchip,rk3229"
        self.inventory["profile_plan"]["expected_dt"] = "rockchip,rk3229"
        self.inventory["emmc_identity_hashes"] = {"mmcblk0_cid_sha256": "b" * 64}
        self.inventory["source_policy"] = {"id": "rk3229-c-backup-last-1",
            "expected_identity_bound": True, "deferred_source": "mmcblk0p10",
            "reason": "observed_source_changed_during_reread", "whole_user_area_allowed": False}
        disk = "/sys/devices/platform/soc/mmc_host/mmc0/mmc0:0001/block/mmcblk0"
        self.inventory["blocks"].extend([
            {"name": "mmcblk0", "device": "/dev/block/mmcblk0", "major_minor": "179:0",
             "kind": "emmc_user_area", "bytes": 7818182656, "sysfs": disk, "parent": "",
             "aliases": [], "holders": [], "read_only_device": False},
            {"name": "mmcblk0p10", "device": "/dev/block/mmcblk0p10", "major_minor": "179:10",
             "kind": "partition", "bytes": 67108864, "start_sector": 196608,
             "sysfs": disk + "/mmcblk0p10", "parent": "mmcblk0", "holders": [],
             "read_only_device": False,
             "aliases": ["/dev/block/platform/30020000.rksdmmc/by-name/backup"]}])
        self.inventory["omitted"] = [{"Name": "mmcblk0p10", "Reason": "mounted_rw_fixture"}]
        self.report["omitted"] = [{"name": "mmcblk0p10", "reason": "mounted_rw_fixture"}]
        self.result["omitted_count"] = 1
        self.save_report()
        write_json(self.root / "inventario.json", self.inventory)

    def test_c_policy_and_prior_reader_contract(self):
        self.with_c_policy()
        answer = V.verify_capture(self.root)
        self.assertEqual(answer["status"], "selected_sources_verified_pc")
        self.assertFalse(answer["all_device_storage_copied"])

    def test_c_policy_rejects_wrong_identity_scope_and_deferral(self):
        self.with_c_policy()
        original = copy.deepcopy(self.inventory)
        changes = [("id", "other"), ("expected_identity_bound", False),
                   ("expected_identity_bound", 1), ("deferred_source", "mmcblk0p9"),
                   ("reason", "ignore_hash"), ("whole_user_area_allowed", True)]
        for key, value in changes:
            with self.subTest(key=key, value=value):
                inv = copy.deepcopy(original); inv["source_policy"][key] = value
                with self.assertRaises(V.InvalidCapture): V.validate_source_policy(inv)
        for target, key, value in [("root", "dt_identity", "another"),
                                  ("root", "emmc_identity_hashes", {}),
                                  ("backup", "start_sector", 0),
                                  ("backup", "aliases", []),
                                  ("backup", "bytes", 4096)]:
            with self.subTest(target=target, key=key):
                inv = copy.deepcopy(original)
                node = inv if target == "root" else inv["blocks"][-1]
                node[key] = value
                with self.assertRaises(V.InvalidCapture): V.validate_source_policy(inv)

    def test_c_policy_rejects_whole_disk_and_wrong_backup_order(self):
        self.with_c_policy()
        valid = self.report["sources"][0]["source"]
        V.validate_policy_source(self.inventory, valid)
        for field, value in [("name", "mmcblk0"), ("major_minor", "179:0"),
                             ("kind", "emmc_user_area")]:
            with self.subTest(field=field):
                row = dict(valid); row[field] = value
                with self.assertRaisesRegex(V.InvalidCapture, "whole_source_in_deferred_report"):
                    V.validate_policy_source(self.inventory, row)
        backup = dict(valid, name="mmcblk0p10", major_minor="179:10")
        row = {"verification_order": "source_source_destination"}
        V.validate_policy_source(self.inventory, backup, row, 13, 14)
        with self.assertRaisesRegex(V.InvalidCapture, "backup_not_last"):
            V.validate_policy_source(self.inventory, backup, row, 0, 14)
        with self.assertRaisesRegex(V.InvalidCapture, "backup_verification_order_mismatch"):
            V.validate_policy_source(self.inventory, backup, {}, 13, 14)
        with self.assertRaisesRegex(V.InvalidCapture, "unexpected_verification_order"):
            V.validate_policy_source(self.inventory, valid, row, 13, 14)
