...
$taskSummary = Join-Path $PSScriptRoot 'reconocimiento-03-estado.json'
...
function Check-Build([string]$expectedReceiptHash = '') {
    $receiptPath = Local-Path 'diagnostico/reconocedor-0.3/COMPILACION.json'
    ...
    if ($build.state -cne 'built_verified_pc' -or $build.version -cne '0.3' -or
    ...
    $sourceDir = Join-Path $taskRoot 'diagnostico/reconocedor-0.3/src'
    ...
    foreach ($property in $properties) {
        if (-not $property.Name.StartsWith('diagnostico/reconocedor-0.3/src/',[StringComparison]::Ordinal) -or
    ...
    $allowedBase = Join-Path $taskRoot 'diagnostico/reconocedor-0.3/privado'
    if (-not $apk.StartsWith($allowedBase+'\',[StringComparison]::OrdinalIgnoreCase) -or
        [IO.Path]::GetFileName($apk) -cne 'Reconocimiento-TVBase-0.3.apk' -or $build.bytes -gt 512MB) { throw 'Ruta de APK fuera de la salida autorizada.' }
...
function Save-State {
    ...
    $public = [ordered]@{schema='tvbase-recognition-usb-delivery-1';state=$taskState.state;version='0.3';
        ...
        free_bytes_after=$taskState.free_bytes_after;reports_preserved=$true;plan_files_added=0;
...
if (Test-Path -LiteralPath $taskSummary) { throw 'Ya existe recibo03; no repetir preparación.' }
$taskBuild = Check-Build
$taskGuide = Local-Path 'diagnostico/reconocedor-0.3/LEEME-USB.txt'
...
$taskNew = @(
    [pscustomobject]@{Source=$taskBuild.Apk;Relative='TVBASE-RECONOCIMIENTO/Reconocimiento-TVBase-0.3.apk';Bytes=$taskBuild.Bytes;Hash=$taskBuild.Sha256},
    [pscustomobject]@{Source=$taskGuide;Relative='TVBASE-RECONOCIMIENTO/LEEME-0.3.txt';Bytes=$taskGuideSize;Hash=$taskGuideHash}
)
...
[long]$taskNewBytes = $taskBuild.Bytes+$taskGuideSize
...
if ($taskReadOnly) {
    [ordered]@{state='checked_only';usb_written=$false;new_files=2;source_files_checked=$taskBuild.SourceCount;
        ...
        planned_new_bytes=$taskNewBytes;old_files_removed=0;tv_modified=$false} | ConvertTo-Json
    return
}
$taskPrivateBase = Join-Path $taskRoot 'privado'
Assert-NoReparse $taskPrivateBase $true
$taskPrivate = Join-Path $taskPrivateBase ('reconocimiento-usb03-'+(Get-Date -Format 'yyyyMMdd-HHmmss')+'-'+[guid]::NewGuid().ToString('N').Substring(0,8))
