...
$taskResult = Join-Path $PSScriptRoot 'evidencia-06-estado.json'
$taskRun = Join-Path $PSScriptRoot ('evidencia-06-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
try {
    ...
    $apk = Join-Path $taskRoot 'rom-simplificada/compilacion/acceso-usb-0.6/acceso-usb.apk'
    $proof = Join-Path $taskRoot 'rom-simplificada/instalador/EVIDENCIA-TESTS-0.6.json'
    if (-not (Test-Path -LiteralPath $proof)) { throw 'Falta evidencia de pruebas 0.6' }
    $tests = Get-Content -LiteralPath $proof -Raw | ConvertFrom-Json
    if ($tests.version -ne '0.6' -or $tests.state -ne 'passed' -or @($tests.adb).Count -lt 12 -or @($tests.shell_syntax).Count -ne 5 -or @($tests.shell_fixtures).Count -lt 15 -or @(@($tests.adb) + @($tests.shell_syntax) + @($tests.shell_fixtures) | Where-Object passed -ne $true).Count -ne 0 -or -not $tests.external_host_rejected -or $tests.guard_result -notmatch '^GUARDS_OK:' -or $tests.max_actual_open_bytes -gt 4096) { throw 'Pruebas 0.6 incompletas o fallidas' }
    $release = Get-Content -LiteralPath (Join-Path $taskRoot 'rom-simplificada/compilacion/acceso-usb-0.6/componente.json') -Raw | ConvertFrom-Json
    if (@($tests.mksh_syntax).Count -ne 5 -or @($tests.mksh_syntax | Where-Object passed -ne $true).Count -ne 0 -or $tests.mksh_default_hash_alias -notmatch 'alias -t') { throw 'Falta prueba representativa de mksh con su alias activo' }
    foreach ($requiredCase in @('real_mksh_binary_with_hash_alias','helper_empty_success_external_guard','helper_malformed_external_guard','blank_sidecar_rejected','selftest_empty_sha_before_certificate','certificates_then_ota_no_candidate_limit','gms_path_rejected_without_splits','missing_required_report','valid_sha_but_wrong_copy','final_mandatory_ota_absent','final_all_required_valid')) {
    ...
    if ($release.version -ne '0.6' -or $release.purpose -ne 'diagnostico_sin_reinicio' -or (Hash-File $apk) -ne $release.sha256 -or $tests.apk_sha256 -ne $release.sha256 -or (Get-Item -LiteralPath $apk).Length -ne $release.bytes) { throw 'APK no coincide con release comprobada' }
    ...
    if ($LASTEXITCODE -ne 0 -or ($badging -join "`n") -notmatch "versionCode='6' versionName='0.6'") { throw 'No es APK0.6' }
    ...
    Copy-Checked $apk (Join-Path $mount 'AccesoUSB-0.6.apk') (Hash-File $apk)
    $readme = Join-Path $taskRoot 'rom-simplificada/instalador/LEEME-EVIDENCIA-0.6.txt'
    $now = Join-Path $mount 'LEEME-AHORA.txt'
    if ((Test-Path -LiteralPath $now) -and (Hash-File $now) -ne (Hash-File $readme)) {
        if ((Hash-File $now) -ne 'b1058f4a28dd99dcec33bcfcf4ad37c3bab8f856baef8ded2a824b93e8e20370') { throw 'LEEME actual desconocido' }
        $old = Join-Path $mount 'LEEME-0.5-retirado.txt'
    ...
    $oldApk = Join-Path $mount 'AccesoUSB-0.5.apk'
    if (Test-Path -LiteralPath $oldApk) {
        if ((Hash-File $oldApk) -ne '7e251f9d202c6ac9407734f2ca4c19077a8b02994f3a115205fdc10cf21b0874') { throw 'APK0.5 distinta' }
    ...
    $taskState.detalle='Acceso USB0.6 de recopilacion sin reinicio copiado y leido. ROM/recovery conservados; informes originales intactos.'
    Save-State
